...
const EndingDialog: React.FC<Props> = ({open, setOpen, won, src, imgSrc, en, jp, romaji, producer, spotifyLink, youtubeLink, appleMusicLink}) => {
    ...
    return (
        <div ref={endingRef} className="h-screen w-full flex fixed justify-center items-center top-0 left-0 right-0 bottom-0 bg-transparent/30 backdrop-blur-lg z-20 duration-500" onClick={onClick}>
            <div className='container h-fit w-fit bg-slate-800 border-slate-700 border rounded-md p-8 flex flex-col space-y-2 items-center justify-center text-center shadow-lg' onClick={preventClose}>
                ...
                <AudioPlayer
                    ...
                    volume={0.5}
                    preload={"auto"}
                    className="rounded-md shadow-lg bg-cyan-50 border-2 w-full h-fit duration-300"
                />

                <div className="container flex flex-row h-fit w-fit space-x-4 p-4">
                    <a href={spotifyLink} target="_blank" className="container w-fit flex flex-row space-x-4 bg-green-600 text-white text-lg hover:bg-green-700 py-4 px-6 rounded-xl shadow-lg items-center text-center duration-300">
                        <FaSpotify className="w-10 h-10"/>
                        <p>Spotify</p>
                    </a>
                    <a href={youtubeLink} target="_blank" className="container w-fit flex flex-row space-x-4 bg-red-700 text-white text-lg hover:bg-red-800 py-4 px-6 rounded-xl shadow-lg items-center text-center duration-300">
                        <FaYoutube className="w-10 h-10"/>
                        <p>YouTube</p>
                    </a>
                    <a href={appleMusicLink} target="_blank" className="container w-fit flex flex-row space-x-4 bg-rose-600 text-white text-lg hover:bg-rose-700 py-4 px-6 rounded-xl shadow-lg items-center text-center duration-300">
